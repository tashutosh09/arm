            ...
            switch (req.params.action) {
                case 'count':
                    {
                        if (req.body) {
                            let conditions = [];
                            if (req.body.RuleID) {
                                conditions.push(`${Constants_1.Constants.TABLES.RULE.key}='${req.body.RuleID}'`);
                            }
                            if (req.body.QueryID) {
                                conditions.push(`${Constants_1.Constants.TABLES.RULE_QUERY.key}='${req.body.QueryID}'`);
                            }
                            if (conditions.length > 0) {
                                Database_1.default.query(DbQueries_1.DbQueries.getRunRuleHistoryCount(conditions)).then((result) => {
                                    res.send({
                                        status: true,
                                        results: result
                                    });
                ...
                case 'get':
                    if (req.body) {
                        ...
                        Logger_1.Loggers.serviceGtddlLog.info(`line case get for view history username ${username2}`);
                        let conditions = [];
                        Logger_1.Loggers.serviceGtddlLog.info(`line case get for view history`);
                        if (req.body.RuleID) {
                            conditions.push(`${Constants_1.Constants.TABLES.RULE.key}='${req.body.RuleID}'`);
                        }
                        if (req.body.QueryID) {
                        ...
                        if (conditions.length > 0) {
                            ...
                            console.log("===" + DbQueries_1.DbQueries.getRunRuleHistories(conditions, req.body.Limit, req.body.Offset, username2));
                            Database_1.default.query(DbQueries_1.DbQueries.getRunRuleHistories(conditions, req.body.Limit, req.body.Offset, username2)).then((result) => {
                                for (var i = 0; i < result.length; i++) {
                                    ...
                                    Logger_1.Loggers.serviceGtddlLog.info(`line history run [${JSON.parse(result[i].RuleParam)['param.source.dbname']}]`);
                                    ...
                                    Logger_1.Loggers.serviceGtddlLog.info(" Inside Router Rule Run History " + " event id " + tRuleID + " event message " + JSON.stringify(tMessage));
                                    if (i == 0) {
                ...
                case 'add':
                    ...
                    if (req.body && req.body.rulesToRun && req.body.rulesToRun.length > 0) {
                        ...
                        for (let item of req.body.rulesToRun) {
                            Logger_1.Loggers.serviceGtddlLog.info(`reached add for rulerun history ${JSON.stringify(item)}`);
                            if (item.RuleRunID
                                ...
                                let batchID = uuidv1();
                                // Creating row
                                Logger_1.Loggers.serviceGtddlLog.info(`reached add for rulerun history parameters recieved ${JSON.stringify(item.RuleParam)}`);
                                values.push(`
                                (
                                    '${item.RuleRunID}',
                                    '${batchID}',
                        ...
                        if (values && values.length > 0) {
                            ...
                            Database_1.default.query(DbQueries_1.DbQueries.insertRunRuleHistory(values)).then((result) => {
...
exports.RouterRuleRunHistory = RouterRuleRunHistory;
